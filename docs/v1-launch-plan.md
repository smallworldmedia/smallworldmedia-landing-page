...
| 6 | **Mobile + reduced-motion audit** | ◐ `8d39f41`→`c5915b8` | The 08-30→31 MOBILE PASS shipped (Nathan remote-reviewing the branch preview): tap-only enter_world + centered globe every breakpoint, nav-carried lockup, uncapsuled 3-line mobile tagline (menu-resident off-home), one chip family (steppers = primary prev/next, ticker next-chip, minimal status bar site-wide, white loader capsule + pct), home→/work rides the enter-tune passage + letter exits, pill open/close states + touch-hover fix, mobile tokens throughout. REMAINING = the real-hardware half: fps gate, scroll-trigger sensitivity field pass, Chrome fp-card centering repro — see memory notes. **FP pager rework: Nathan picked the SCALE 09-02; revision rounds 1–10 built + probe-verified through 09-04, CHECKPOINTED `2d59980`** (scrim + baked blur/tint/grain recipe + `?scrimtune` bench, wheel wrap + seams, curved screen-space hairline, single-instance selection with the near trio + flipper box + tickers, pause screen with the rotated accent-chip label, Turn-at-disengage + thaw-at-commit, pulse metronome, mobile corner preset for the media grid + z-fight epsilon, mobile grain/card bakes + dial sets) — next = further refinement per Nathan's device pass, THEN the bake (delete tape/tuner + legacy rail); spec + per-round open calls in `docs/fp-pager-rework-approaches.md`. |
