...
| 6 | **Mobile + reduced-motion audit** | ◐ `8d39f41`→`c5915b8` | The 08-30→31 MOBILE PASS shipped (Nathan remote-reviewing the branch preview): tap-only enter_world + centered globe every breakpoint, nav-carried lockup, uncapsuled 3-line mobile tagline (menu-resident off-home), one chip family (steppers = primary prev/next, ticker next-chip, minimal status bar site-wide, white loader capsule + pct), home→/work rides the enter-tune passage + letter exits, pill open/close states + touch-hover fix, mobile tokens throughout. REMAINING = the real-hardware half: fps gate, scroll-trigger sensitivity field pass, Chrome fp-card centering repro — see memory notes. **FP pager rework: the SCALE arm is THE pager (09-07, `adb6d14` pushed)** — rounds 1–10 (`2d59980`) + Nathan's device-pass rounds 11 a–h (`860dc7a`…`8703218`: dialable falloff + ink + stagger curves, sliced axis, viewport-evaluated name caps, detent-keyed sizes, pointer on the flipper spring, mobile wall re-tier + texture-res fix, wall-clock-locked tickers HOUSE-WIDE); tape + tuner DELETED (`d6a04fd`); scale is the DEFAULT and SSR'd (`8bdb013`, `a6553b9` — no rail flash; `?pager=rail` = the legacy rail, kept only as the parked rail-arm proposal's front end); mobile chip-wipe fixed (own 520ms ease); desktop name cap 20rem. **09-07 desktop round (`b57c853`…`11be6d4` pushed):** Sanity title sub-token after the client name; fixed-width desktop box + always-rolling FILL ticker (Nathan's bake fillpxs 30 / boxgap 12 / namemax 2400 / unfill 0); every `?` dial had been DEAD since the SSR (hydration kept the server's fallbacks — now re-applied at mount); two ticker phase bugs fixed (re-phase on re-fill; the wrap re-base swapping in an unfilled clone — the COCO scroll-up case); services readout tried and DROPPED; desktop chip wipe 300ms. Spec § 09-07 in `docs/fp-pager-rework-approaches.md`; 275 tunables, `--check` PASS. Next = Nathan's next read of the preview; then retire the legacy rail code once the rail arm is dropped. **09-07 (late): client-logo ticker** — the band riding the TOP of the links footer on all three footer routes (Nathan): copy + vertical odometer word ticker (artists/promoters/labels/agencies/venues) + ONE row of 40 white-on-transparent client logos rolling one direction at a constant px/s; fades on a window over `--footer-reveal` (no second listener), animations gated on `data-footer-revealed`; `/work` retracts the footer on pager engage so the scrim never dims the row; assets static via `scripts/prep-client-logos.mjs` (intake `Client Logos/` → `src/assets/client-logos/`, Sanity later); 9 `?logo*` dials (09-08: `?logobal ?logoref` optical balance — squares up, wordmarks down). **09-08 chrome round (`b7b51fa`…`5eea54a` pushed):** ticker on THE drag + momentum engine (`src/lib/dragMomentum.js`, the globe's); home reveals the footer on scroll; footer exits MASKED (`--footer-peak`, per-frame clip of the tagline stack); tagline pill raises the footer IN PLACE (house-glide wipe, click-outside closes); one overlay wipe (`overlayWipe.js`: inquiry FLIPPED bottom-up, mobile menu, privacy); privacy = an overlay whose pill is the close; mobile footer = lockup + © group + privacy pill twins; inquiry lockup at the nav seat. Process round 1: keyword highlight wipe (`keywords.jsx`, `[[markers]]`), footer baked into the last slide, scrim full-bleed + OFF (`?scrim`), phone cards bottom-anchored, no text shadows. 286 tunables, `--check` PASS. NEXT = process page round 2. |
...
| F18 | Next-project band (detail continuation) | ✅ | `NextProjectBand` at the detail-page bottom (Figma "next project card" 15:385/401/417): next project's identity + first-order media (live HLS, poster-first), the house resistance accumulator gated to the document end (arming beat `?nparm` guards flick tails — unlike / and /work the gesture arrives mid-flight), staggered label→title→media drag choreography, commit = Envelopment passage (`swm:envelop` → navigate → arriving mount releases) + `swm:worldIndex` sync so the breadcrumb returns to the World you rode into. Chain = /work World order, wraps last→first. Knobs `?scroll ?npms ?npcover ?nppre ?npsplit ?nplag ?npzoom ?nparm`. 21-check Playwright suite (desktop drag/release/commit, wraparound, RM, mobile stacked). |
