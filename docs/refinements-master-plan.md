...
| A2 | House scroll tension wiring | AGENT | ✅ this PR | Per-surface constants in `motion.js` (`SCROLL_TRIGGER_HOME_PX` 600 / `_WORK_PX` 600 / `_PROCESS_PX` 700 — the 700 is deliberately heavier, not flattened) + shared `TOUCH_GAIN`(2)/`RELEASE_MS`(160) consumed by `Hero.jsx` + `FeaturedProjects.jsx`. `smoothScroll.js` spreads `LENIS_TUNING` (empty = today's feel). Process branch consumes `_PROCESS_PX` + its own query param at rebase. |
| A2b | Scroll tension **dial-in** | CREATIVE | ☐ | Nathan session: soften trigger values + fill `LENIS_TUNING`. Live dial wired: `?lerp` / `?wheelmult` / `?lenisdur` (unset = shipped feel) on every Lenis document-scroll route; blessed values bake into `LENIS_TUNING` (`motion.js`, candidate starting points in its comment block). ⚠️ Lenis feeds Grid Socket parallax + orbit scroll-kick — regression pass on document-scroll routes required. |
...
| ID | Item | Class | Status | Notes |
|----|------|-------|--------|-------|
| B1 | Copy deck v2 | AGENT | ☐ | `processContent.js`: h1 → "PROCESS", sub/cue retired, new bottom-right mono tagline ("An inside look at our approach…"), caps-word headlines + snake_case chips, all 5 Notion blurbs verbatim (S2 exceeds the ≤45-word budget — keep, amend comment), CTA line swap. Supersedes the dirty stage-04 working-tree edit. |
| B2 | Chrome moves | AGENT | ☐ | **[previous] only** → top-left below nav (consume bar-height + `--nav-inset` tokens, no new literals); [next] stays bottom-left; retire/retune the debug-shove rule. Meter → bottom-center, `CELLS` 10→20; base rule drops `right`, adds `left:50%/translateX(-50%)`; ≤768 override must add `transform:none`. Tagline bottom-right, data-bg-aware, hidden ≤768. Gate: collision pass 768/1024/1280 + `?debug` open. |
| B3 | Thread → 3D | CREATIVE ⭐ | ☐ | Promote the DOM-SVG thread to an in-scene `THREE.Line`/tube, `depthTest:true` — occluded by panels + `innerSphere`. Inside-normal hops, no intermediate vertices. Retires the screen-space string (amend spec §3). Unlocks B5+B6. |
| B4 | S1 point cloud + decoys + labels | CREATIVE ⭐ | ☐ | Flicker-in/scale-up entrance (snappy, randomized); slow varied-speed point-cloud rotation (replaces free tumble); min-distance seeding (zero collisions); decoys as **one InstancedMesh** (separate pool — draw-call budget ≤90); DOM blob-tracking mono labels via a shared 3D→screen helper + rolling Notion term list. |
| B5 | S2 zoom + drop-out + trim path | CREATIVE | ☐ | Rides B3+B4: scroll-in zoom, decoys flicker out to the final 84, occluded trim-path draw. |
| B6 | S3 assembly + zoom-out + occlusion | CREATIVE | ☐ | Assembly on quantizer commit; zoom-out on the house falloff; thread interior (B3) — remove the fade-out workaround. |
| B7 | S4 weight/lag | CREATIVE | ☐ | More dynamic scale-up offsets; slight lag behind scroll for weight. End-scale unchanged. |
| B8 | S5 zoom-in + tilt + inner stroke + pulse migration | CREATIVE | ☐ | Zoom into the S5 pose (resize-safe); ~2:00 tilt via rhythm-driven scalar in the tick; brown-blue `uStrokeMix` inverse to `uPower` (new color — swatches in-session); envelope onto the house pulse behind `?decaycurve` A/B (expo stays default until the feel checkpoint passes). |
| B9 | Hero splash + globe-O | CREATIVE | ☐ | Blue field + globe, "PROCESS" via SplitText chars `stagger:{from:'random'}`, ~1.5 s hold, out into stage-01; RM still-path. Globe-O = absolute overlay tracked to the glyph (SVG placeholder + slow rotation); final Lottie (looping longitudinal a→b) = separate asset task. |
