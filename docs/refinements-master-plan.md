...
| NAV-2 | Nav micro-interactions ×4 | CREATIVE | ◐ PR #38 (preview) | Built 2026-07-17: `?navfx=1..4` behind a flag (default nav byte-unchanged), desktop + mobile — (1) glyph awake, (2) scramble pass, (3) bracket carets, (4) kinetic rule. Net-new current-page state (`data-current`+`aria-current`, `/work`+`/work/*`→featured_projects, `/process`→process, refreshed on after-swap since the island persists). Verified: all 4 render + mark current, envelop stagger clean (decorative nodes inside anchors / sibling overlay), after-swap survives clearProps, focus-visible + RM, 0 errors. **Nathan picks one on preview → promote (drop flag) + drop the other 3.** |
...
| FP-1 | enter_world pulse (dims) | CREATIVE | ◐ PR #39 (preview) | Built 2026-07-17: first consumer of `housePulseLoop` — CTA rests at 1, dips to 0.62 on the house pulse, starts after the entrance rests (`?fp1dim`/`?fp1period`/`?fp1rest`). Sync-created (context-killed on unmount) + phase-guarded so Turn teardown leaves no stranded dim; hover/focus suppress. Verified: holds 1.0 to ~2.0s → 0.62 troughs @3.000s, hover recovers 171ms, clean Turn teardown, knobs + RM, 0 errors. Re-aligns the stale JSDoc; retires "primaries rest still". Nathan sign-off on preview. |
