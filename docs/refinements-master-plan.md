...
| FP-1 | enter_world pulse (dims) | CREATIVE | ◐ PR #39 (Nathan dialing curve) | Pulse shipped (verified). Added a **?fp1tune=1 house-pulse-curve bench** on /work (house-debug idiom): live sliders (attack peak/softness, hold, fall, period, dim, rest, on-ratio), live SVG plot, readonly HOUSE_PULSE_PATH, copy_values/copy_url. Dev-only/gated — shipped pulse byte-identical without the param. Fixed bench defects: hydration-safe mount + clamp path monotonic (kills "Invalid CustomEase"). **Nathan dials → pastes the block → I bake it into motion.js as the house curve token.** |
