...
| `?tagsize` | the readout's TYPE size as an em ratio of the row (the roster size × the row's curve — same basis as `?sub`); the line height stays `?tagsh`, so a big value crowds the box | `0.62` (aliases `--scale-sub`) | > 0 | `global.css --scale-tags-size` |
...
| `?unfill` | scale 09-07: how long the OUTGOING row keeps its fill roll after the detent, in ms — the flipper is still deflected onto it at the threshold, so an instant drop snapped the roll to a single name inside the box (COCO); 3τ (300) hung too long outside it | ≈ `?fliptau` × 1000 (`100`), floor 60 | ≥ 0 (`0` = drop at once) | `GraticulePager.jsx unfillLater` |
