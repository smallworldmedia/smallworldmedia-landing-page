...
| `?pager` | pager arm: lazy-swaps a skin in for the legacy `.fp-pager` rail after mount (SSR + first paint always render the rail). **Since 09-06 the SCALE arm is the DEFAULT** — no param = `scale` (the graticule, built 09-01 over the shared engine `pager/usePagerGesture.js`; spec in `docs/fp-pager-rework-approaches.md`). `rail` (or any unknown value — the lookup is `Object.hasOwn`) keeps the legacy rail. The `tape` and `tuner` arms were DELETED 09-05 | `scale` | `scale` \| `rail` | default flipped 09-06; the rail stays as the parked rail-arm proposal's front end |
