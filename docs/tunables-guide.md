...
| `?namemax` | scale 09-07 (Nathan: desktop names cut off too suddenly): the client-name cap in px — writes both `--scale-name-max` and `--scale-name-sel-max` (one cap, no jump at the detent). Desktop token 12rem → **20rem** (fits the widest name at roster 1.9 with no ticker); ≤768 the cap is evaluated from the viewport and this overrides it | desktop `320` (20rem) · ≤768 evaluated | px > 0 | `global.css --scale-name-max` |
