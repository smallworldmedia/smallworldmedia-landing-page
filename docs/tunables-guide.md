...
| `?selscale` | `?pager=scale` r11e: the SIZE the curve lands rows at — the ±hold rows' scale and the base of the selected row (`--scale-sel`); the box, the evaluated ≤768 caps and the stagger read the same token | `1.8` (**baked 09-05 r11f**, Nathan's dial; was 1.688 = `--text-secondary`/`--text-mono`) | > 1 | `global.css --scale-sel` |
...
| `?falloffexp` | scale r11: the roll-off's curve exponent past the hold (`pow(wr, exp)`) — 1 = linear, 2 = f²; the ink (55% → 100% white) rides the same weight | `2` (**baked r11f**; r11b 1, r11 1.5) | > 0 | `global.css --scale-falloff-exp` |
