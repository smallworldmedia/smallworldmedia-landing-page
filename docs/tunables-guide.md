...
| `?pager` | pager arm: lazy-swaps a skin in for the legacy `.fp-pager` rail after mount (SSR + first paint always render the rail). `scale` = the graticule, built 09-01 over the shared engine (`pager/usePagerGesture.js`); spec in `docs/fp-pager-rework-approaches.md`. The `tape` and `tuner` arms were DELETED 09-05 (Nathan's call — the scale arm won). Unknown values (and prototype keys — the lookup is `Object.hasOwn`) fall through to the legacy rail | legacy rail | `scale` | arm gate until the rail is retired or the rail arm (see spec § 09-05) ships |
