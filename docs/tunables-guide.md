...
2026-09-01 (`feature/v1-launch` @ `1e45cda` plus the uncommitted `?pager=tape`
build): 229 URL params across 26 reader files, 10 bench/debug panels, 4 debug
globals. Regenerate it with the recipe at the bottom; `node scripts/tunables-keys.mjs --check`
...
| `?pager` | pager A/B arm: `tape` lazy-swaps `TapeWheelPager` in for the legacy `.fp-pager` rail after mount (SSR + first paint always render the rail). `scale` (graticule) and `tuner` (signal tuner) are spec'd in `docs/fp-pager-rework-approaches.md`, not built; any other value = legacy rail | legacy rail | `tape` | variant gate until a skin is blessed |
| `?detent` | tape wheel: finger-travel px per station while engaged; also the flick-velocity divisor (`?pager=tape` only, per-render) | `56` px | any finite; **no clamp** (0 → NaN) | `motion.js:100 PAGER_DETENT_TOUCH_PX` |
| `?wheeldetent` | tape wheel: desktop wheel deltaY px per station (`hover:hover` + `pointer:fine` only) | `90` px | any finite; no clamp | `motion.js:101 PAGER_DETENT_WHEEL_PX` |
...
Pager constants with **no** URL knob (edit `motion.js:98-108`): `PAGER_HOLD_MS` 200, `PAGER_SLOP_PX` 8, `PAGER_STALL_COMMIT_MS` 450, `PAGER_PEEK_MS` 900, `PAGER_FLICK_CARRY_S` 0.12, `PAGER_TAU_SCRUB` 0.05, `PAGER_TAU_GLIDE` 0.27, `PAGER_END_RESIST` 0.3, `PAGER_KEY_COMMIT_MS` 400. Engine-local: `TAU_SETTLE` 0.08, `READ_BEAT_MS` 240, `HOVER_INTENT_MS` 120, `END_OVERTRAVEL_CAP` 0.35, `CLICK_SUPPRESS_MS` 350 (`usePagerGesture.js:42-46`). Tape chrome tokens: `--tape-w` `--tape-row` `--tape-rows` `--tape-open-ms` `--tape-close-ms` `--tape-name-max` `--pager-stage-dim` (`global.css:239-249`; ≤768 tier `:2612-2615`). Other /work constants: `TOUCH_GAIN` 2, `RELEASE_MS` 160, `GLIDE_MS` 800 (`motion.js:80-89`), FeaturedProjects `PAGER_BASE_GAIN` 1.6 / `PAGER_HOVER_GAIN` 1.8 / `CTA_MAX_EXTRA` 0.3 / `HOME_PRE_COVER` 0.4 / `RM_WHEEL_THRESHOLD` 60 (`FeaturedProjects.jsx:62-71`).
...
6. `?detent` / `?wheeldetent` / `?scroll` / `?caret` have no clamps; `0` yields NaN / Infinity.
