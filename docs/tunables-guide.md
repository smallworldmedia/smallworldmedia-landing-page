...
| `?pager` | pager A/B arm: lazy-swaps a skin in for the legacy `.fp-pager` rail after mount (SSR + first paint always render the rail). `tape` = the tape wheel, `scale` = the graticule, `tuner` = the signal tuner — all three built 09-01 over one shared engine (`pager/usePagerGesture.js`); specs in `docs/fp-pager-rework-approaches.md`. Unknown values (and prototype keys — the lookup is `Object.hasOwn`) fall through to the legacy rail | legacy rail | `tape` \| `scale` \| `tuner` | variant gate until an arm is blessed |
| `?detent` | finger-travel px per station while engaged, and the flick-velocity divisor. Read per-render by whichever arm is mounted. On `scale` it ALSO re-pitches the visual scale (the skin writes it back as the inline `--scale-pitch`), so touch stays exactly 1:1 with the ticks | tape `56` · scale `48` both tiers (09-04 r8 vertical-space bump — was 44 desktop) · tuner `56` px | any finite; **floored at 8px** in all three arms (09-01 — `0` used to divide by zero → NaN landing) | `motion.js PAGER_DETENT_TOUCH_PX` / `SCALE_PITCH_PX` + `SCALE_PITCH_MOBILE_PX` |
| `?wheeldetent` | desktop wheel deltaY px per station (`hover:hover` + `pointer:fine` only). Per-render, per arm | tape `90` · scale `100` · tuner `80` px | any finite; floored at 8px | `motion.js PAGER_DETENT_WHEEL_PX` / `SCALE_WHEEL_DETENT_PX` / `TUNER_DETENT_WHEEL_PX` |
| `?idlecommit` | `?pager=tuner` only: desktop idle-at-detent auto-commit. `0` sets the engine's `stallMs` to Infinity (commit on pointerleave / keyboard / blur only); a value above `1` is read as the dwell in ms (floored at 50) | `1` (= `TUNER_IDLE_COMMIT_MS` 650 ms) | `0` \| `1` \| ms > 1 | `motion.js TUNER_IDLE_COMMIT_MS` |
| `?scramblename` | `?pager=tuner` only: whether the seeking scramble covers the client-name plate as well as the digits. `0` = digits only (the judge-flagged double-scramble escape hatch) | `1` (on) | `0` \| anything else = on | taste call, open for Nathan |
...
| `?feather` | `?pager=scale` only (09-02 round 2): the deployed window's feather radius in px at each end (the mask fade). Absent/`0` = the token default `--scale-feather` (TWO pitches since round 2 — was one) | token (2 × pitch = 88/96px) | px > 0 | `--scale-feather` (featured-projects.css root-local) |
...
| `?magnet` | `?pager=scale` only (09-03 round 5): the tension curve's exponent — the rendered strip HANGS on the current station and snaps across the threshold on `sign(f)·\|2f\|^exp/2` (the engine's magnet shape, per-skin since round 5; the tape keeps 1.6). Higher = longer hang + steeper snap; the flipper box rides the same shaped position | `2.2` (`SCALE_MAGNET_EXP` — **baked 09-03 r6**, Nathan's device dial; was 4) | clamped 1–8 (below 1 anti-magnetizes, past 8 the crossing aliases a frame) | `motion.js SCALE_MAGNET_EXP` |
| `?fliptau` | `?pager=scale` only (09-03 round 5): the flipper box's spring τ in seconds — how fast the near-black selected-name chip chases its target (the current name's screen offset; the ±half-pitch jump at the threshold renders through this spring as the game-show-arrow flick). Lower = snappier flick, higher = floatier | `0.005` (`SCALE_FLIP_TAU_S` — **baked 09-03 r6**, Nathan's device dial; was 0.06) | clamped 0.001–1 s (floor lowered r6 so the bake isn't silently clamped) | `motion.js SCALE_FLIP_TAU_S` |
...
| `?t1deg` | corner preset (09-04 r10): tier-1 plate scale × `PLATE_DEG` — under the preset the WALLS ride it too (tier-1 footprint parity) | `1.35` (r9 was 1.8, the desktop wall footprint — Nathan: cut off by the viewport) | > 0 | `worldConfig.js CORNER_T1_MUL` |
| `?t2deg` | corner preset (09-04 r10): tier-2 plate scale × `PLATE_DEG` | `0.8` (r9 was 1) | > 0 | `worldConfig.js CORNER_T2_MUL` |
| `?cornerin` | corner preset (09-04 r10): seat pull-in factor on the wall-anchor corner coords — smaller = tighter to centre ("going out of frame a bit" is fine, heavy viewport cropping is not) | `0.8` (1 = the r9 corners) | > 0, ≤ ~1.2 useful | `worldConfig.js CORNER_INSET` |
...
Pager constants with **no** URL knob, shared by all three arms (`motion.js`): `PAGER_HOLD_MS` 200, `PAGER_SLOP_PX` 8, `PAGER_STALL_COMMIT_MS` 450, `PAGER_PEEK_MS` 900, `PAGER_FLICK_CARRY_S` 0.12, `PAGER_TAU_SCRUB` 0.05, `PAGER_TAU_GLIDE` 0.27, `PAGER_END_RESIST` 0.3, `PAGER_KEY_COMMIT_MS` 400. Per-arm, no knob: `SCALE_END_RESIST` 0.2 (`SCALE_STALL_COMMIT_MS` deleted 09-03 r6 — the scale passes `stallMs: Infinity`, so a desktop wheel settle is a preview and the Turn evaluates only at disengage: pointerleave / touch release / keyboard); `TUNER_LOCK_WINDOW` 0.33 (|q − round(q)| at or under this = LOCKED, beyond = SEEKING), `TUNER_SCRAMBLE_MS` 35 (the `?txchar` beat), `TUNER_HAPTIC_MS` 8. Engine-local: `TAU_SETTLE` 0.08, `READ_BEAT_MS` 240, `HOVER_INTENT_MS` 120, `END_OVERTRAVEL_CAP` 0.35, `CLICK_SUPPRESS_MS` 350 (`usePagerGesture.js`).

Engine `tuning` options a skin may pass (09-01, all optional and defaulting to the tape's behaviour): `detentPx`, `wheelDetentPx`, `endResist` (0 = hard clamp, the tuner), `stallMs` (non-finite = never auto-commit), `magnet` (false = 1:1, no detent shaping — both new arms). `clampRaw()` bounds raw overtravel to `END_OVERTRAVEL_CAP / endResist`, and `focusSilently(el)` moves a roving tabstop without arming the `:focus-visible` engage.

Chrome tokens — tape: `--tape-w` `--tape-row` `--tape-rows` `--tape-open-ms` `--tape-close-ms` `--tape-name-max`; scale: `--scale-w` `--scale-num-h` `--scale-pitch` (pre-hydration fallback — the skin writes it inline) `--scale-rows` (deployed aperture, stations each side of the lens: 7 desktop / 6 ≤768) `--scale-open-ms` `--scale-close-ms` `--scale-stagger-ms` (a CAP — the effective delay is fitted to the open/close budget) `--scale-fade-ms` `--scale-flash-ms` `--scale-lens-zoom` `--scale-label-max`; tuner: `--tuner-w` `--tuner-h` `--tuner-strip` `--tuner-open-ms` `--tuner-close-ms` `--tuner-name-max`; shared: `--pager-stage-dim`. All in `global.css` `:root` with a ≤768px tier. Runtime-written root properties (not authored tokens): `--scale-c` (the row under the lens now), `--scale-q` (where the lens landed — the hairline's close origin), `--tuner-pos` (0..1 strip fill), `--tuner-glide-ms` (= `TURN_DURATION`). Other /work constants: `TOUCH_GAIN` 2, `RELEASE_MS` 160, `GLIDE_MS` 800 (`motion.js:80-89`), FeaturedProjects `PAGER_BASE_GAIN` 1.6 / `PAGER_HOVER_GAIN` 1.8 / `CTA_MAX_EXTRA` 0.3 / `HOME_PRE_COVER` 0.4 / `RM_WHEEL_THRESHOLD` 60 (`FeaturedProjects.jsx:62-71`).
